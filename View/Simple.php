...
class Simple implements \Syx\ViewInterface {
    /**
     ...
     * Syx\View\Simple::assign('name', $value) assigns a variable called 'name'
     * with the corresponding $value.
     * Syx\View\Simple::assign($array) assigns the array keys as variable
    ...
    public function assign( $name, $value = NULL ) {
        ...
        if ( is_string( $name ) ) {
            ...
            throw new \Syx\Exception( 'assign() expects a string or array, received ' . gettype( $name ) );
        }
     ...
     * @return Syx\View\Simple
     * @throws Syx\Exception\LoadFailed\View if $name is not a string,
     */
    public function assignRef( $name, &$value ) {
        ...
        if ( is_string( $name ) ) {
            ...
            throw new \Syx\Exception( 'assign() expects a string, received ' . gettype( $name ) );
        }
    ...
    public function setScriptPath( $templateDir ) {
        if ( is_string( $templateDir ) && \Syx\G::isAbsolutePath( $templateDir ) ) {
     ...
     * @return string The script output.
     */
    public function evaluate( $tpl_content, $vars = [] ) {
    ...
    protected function _script( $name ) {
        if ( preg_match( '#\.\.[\\\/]#', $name ) ) {
            throw new \Syx\Exception( 'Requested scripts may not include parent ' . 'directory traversal ("../", "..\\" notation)' );
        }
        if ( $this->_tpl_dir == '' ) {
            throw new \Syx\Exception\LoadFailed\View( 'Could not determine the view script path, ' . 'you should call Syx\View\Simple::setScriptPath to specific it' );
        }
        if ( \Syx\G::isAbsolutePath( $name ) && is_readable( $name ) ) {
            return $name;
        } else if ( is_readable( $this->_tpl_dir . DIRECTORY_SEPARATOR . $name ) ) {
            return $this->_tpl_dir . DIRECTORY_SEPARATOR . $name;
        }
        throw new \Syx\Exception\LoadFailed\View( "Unable to find template " . $this->_tpl_dir . DIRECTORY_SEPARATOR . $name );
    }
